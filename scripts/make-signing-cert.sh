...
# change the DR and break the grant again). Ported from macrec's cert script.
#   ⚠ Never delete/regenerate this cert — back up the .p12 below.
set -e

CERT_NAME="MacmailSign"
KEYCHAIN="$HOME/Library/Keychains/login.keychain-db"
BACKUP_DIR="$HOME/.config/macmail"
BACKUP="$BACKUP_DIR/${CERT_NAME}.p12"
P12_PASS="macmail-sign"
...
echo "macmail: importing into the login keychain (-A: let codesign use it without prompting)…"
...
echo "macmail: backed up to $BACKUP (password: $P12_PASS) — keep it; losing it means re-granting FDA."
